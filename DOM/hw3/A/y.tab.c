...
{
  "$end", "error", "$undefined", "LEAF", "'+'", "'-'", "'*'", "'/'",
  "'('", "')'", "$accept", "line", "VERTIC", YY_NULLPTR
};
#endif
...
#line 58 "test.y" /* yacc.c:1646  */
    { 
			...
			dfs((yyvsp[0].index));
			if(st[(yyvsp[0].index)].val>=0)printf("\nthe result is : %d\n", (int)(st[(yyvsp[0].index)].val+0.5));
			else printf("\nthe result is : %d\n", (int)(st[(yyvsp[0].index)].val-0.5));
		}
...
#line 65 "test.y" /* yacc.c:1646  */
    {
			st[top].lc = (yyvsp[-2].index);
			st[top].rc = (yyvsp[0].index);
			st[top].val = st[(yyvsp[-2].index)].val + st[(yyvsp[0].index)].val;
			st[top].s[0] = '+';
			(yyval.index) = top;
			top++;
		}
...
#line 98 "test.y" /* yacc.c:1646  */
    {
			int i=0;
			for(i=0; (yyvsp[0].leaf)[i]!='\0'; i++){
				st[top].s[i] = (yyvsp[0].leaf)[i];
			}
			st[top].s[i] = '\0';
			st[top].val = atoi((yyvsp[0].leaf));
			st[top].lc = -1;
			st[top].rc = -1;
			(yyval.index) = top;
			top++;
